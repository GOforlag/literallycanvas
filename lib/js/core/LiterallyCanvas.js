var INFINITE, JSONToShape, LiterallyCanvas, Pencil, _logSnapshotSize, actions, bindEvents, createShape, math, ref, renderShapeToContext, renderShapeToSVG, renderSnapshotToImage, renderSnapshotToSVG, shapeToJSON, util,
...
_logSnapshotSize = function(snapshot) {
  var bytes, data, i, len, perPoint, points, shape, shapes, size, stale, verdict;
  shapes = (snapshot.shapes || []).concat(snapshot.backgroundShapes || []);
  points = 0;
  stale = 0;
  for (i = 0, len = shapes.length; i < len; i++) {
    shape = shapes[i];
    data = shape != null ? shape.data : void 0;
    if (!data) {
      continue;
    }
    points += (data.pointCoordinatePairs || data.points || []).length;
    if (data.smoothedPointCoordinatePairs) {
      stale += 1;
    }
  }
  bytes = JSON.stringify(snapshot).length;
  size = bytes >= 1048576 ? ((bytes / 1048576).toFixed(2)) + " MB" : ((bytes / 1024).toFixed(1)) + " KB";
  verdict = !shapes.length ? 'empty drawing - draw a pen stroke, then check again' : stale ? stale + "/" + shapes.length + " shapes still carry smoothedPointCoordinatePairs - OLD code is running" : 'no smoothed points serialized - NEW code is running';
  perPoint = points ? " | " + ((bytes / points).toFixed(0)) + " bytes/point (was ~355)" : '';
  return console.log("%c[literallycanvas] snapshot " + size + " | " + shapes.length + " shapes | " + points + " points" + perPoint + "\n  " + verdict, "color: " + (stale ? '#c0392b' : '#27ae60'));
};
