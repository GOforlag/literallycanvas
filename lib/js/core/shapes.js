...
_createLinePathFromData = function(shapeName, data) {
  ...
  if (!points && smoothedPoints) {
    points = smoothedPoints;
  }
  if (!(points != null ? points[0] : void 0)) {
    return null;
  }
...
linePathFuncs = {
  ...
  addPoint: function(point) {
    ...
    if (!this.smoothedPoints || this.points.length < this.sampleSize) {
      ...
      this.smoothedPoints.length = Math.max(0, this.smoothedPoints.length - this.segmentSize * (this.tailSize - 1));
      return (ref2 = this.smoothedPoints).push.apply(ref2, this.tail);
    }
  },
  move: function(moveInfo) {
    var len, len1, pt, q, r, ref2, ref3, results;
    if (moveInfo == null) {
      moveInfo = {};
    }
    ref2 = this.points;
    ...
    if (this.smoothedPoints && this.smoothedPoints !== this.points) {
      ref3 = this.smoothedPoints;
      results = [];
